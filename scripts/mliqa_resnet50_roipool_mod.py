...
train_qual_label = []
for i in range(len(train_img_qual)):
    tmp_label = [train_img_qual[i], train_patch_qual[i]]
    # tmp_label = np.array(tmp_label).T.tolist()
    train_qual_label.append(tmp_label)

val_qual_label = []
for i in range(len(val_img_qual)):
    tmp_label = [val_img_qual[i], val_patch_qual[i]]
    # tmp_label = np.array(tmp_label).T.tolist()
    val_qual_label.append(tmp_label)

test_qual_label = []
for i in range(len(test_img_qual)):
    tmp_label = [test_img_qual[i], test_patch_qual[i]]
    # tmp_label = np.array(tmp_label).T.tolist()
    test_qual_label.append(tmp_label)

df_train['img_dist'] = df_train['img_dist'].apply(ast.literal_eval)
train_img_dist = []
for i in range(len(X_train)):
  y = df_train['img_dist'][i]
  train_img_dist.append(y)

df_val['img_dist'] = df_val['img_dist'].apply(ast.literal_eval)
val_img_dist = []
for i in range(len(X_val)):
  y = df_val['img_dist'][i]
  val_img_dist.append(y)

df_test['img_dist'] = df_test['img_dist'].apply(ast.literal_eval)
test_img_dist = []
for i in range(len(X_test)):
  y = df_test['img_dist'][i]
  test_img_dist.append(y)


df_train['patch_dist'] = df_train['patch_dist'].apply(ast.literal_eval)
train_patch_dist = []
for i in range(len(X_train)):
  y = df_train['patch_dist'][i]
  train_patch_dist.append(y)

df_val['patch_dist'] = df_val['patch_dist'].apply(ast.literal_eval)
val_patch_dist = []
for i in range(len(X_val)):
  y = df_val['patch_dist'][i]
  val_patch_dist.append(y)

df_test['patch_dist'] = df_test['patch_dist'].apply(ast.literal_eval)
test_patch_dist = []
for i in range(len(X_test)):
  y = df_test['patch_dist'][i]
  test_patch_dist.append(y)


train_dist_label = []
for i in range(len(train_img_dist)):
    tmp_label = [train_img_dist[i], train_patch_dist[i]]
    # tmp_label = np.array(tmp_label).T.tolist()
    train_dist_label.append(tmp_label)

val_dist_label = []
for i in range(len(val_img_dist)):
    tmp_label = [val_img_dist[i], val_patch_dist[i]]
    # tmp_label = np.array(tmp_label).T.tolist()
    val_dist_label.append(tmp_label)

test_dist_label = []
for i in range(len(test_img_dist)):
    tmp_label = [test_img_dist[i], test_patch_dist[i]]
    # tmp_label = np.array(tmp_label).T.tolist()
...
def create_dataset(filenames, rois, label_1, label_2, is_training):
    ...
    dataset = tf.data.Dataset.from_tensor_slices(({'input_layer':filenames,'input_rois':rois},
...
train_ds = create_dataset(X_train, X_roi_train, train_qual_label, train_dist_label, is_training=True)
val_ds = create_dataset(X_val, X_roi_val, val_qual_label, val_dist_label, is_training=False)
test_ds = create_dataset(X_test, X_roi_test, test_qual_label, test_dist_label, is_training=False)
...
def build_model():
    ...
    x1 = layers.TimeDistributed(layers.Dense(32, activation='relu', name='hidden_layer2'))(x)
    x1 = layers.TimeDistributed(layers.Dropout(0.2))(x1)
    output_1 = layers.Dense(1, name='output_1')(x1)

    x2 = layers.TimeDistributed(layers.Dense(32, activation='relu', name='hidden_layer3'))(x)
    x2 = layers.TimeDistributed(layers.Dropout(0.2))(x2)
    output_2 = layers.Dense(7, name='output_2')(x2)
    ...
    # x1 = layers.Dense(32, activation='relu', name='fc2')(x)
    # x1 = layers.Dropout(0.2)(x1)
    # img_qual = layers.Dense(1, name='img_qual')(x1)
    # patch_qual = layers.Dense(1, name='patch_qual')(x1)
    #
    # x2 = layers.Dense(32, activation='relu', name='fc3')(x)
    # x2 = layers.Dropout(0.2)(x2)
    # img_dist = layers.Dense(7, name='img_dist')(x2)
    # patch_dist = layers.Dense(7, name='patch_dist')(x2)
